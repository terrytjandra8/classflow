...
export const Home: React.FC<HomeProps> = ({ 
    ...
    onJoinBoard, onNavigateToMake, onOpenSetup, onUpdateBoard, username, userAvatar, userId, theme, isSuperAdmin, selectedClass
}) => {
    
    const randomQuote = useMemo(() => QUOTES[Math.floor(Math.random() * QUOTES.length)], [username]);
    ...
    const {
        ...
        handleConfirmAction,
        handleMenuAction
    } = useBoardBrowser(boards, userId, onDeleteBoard, onEmptyTrash, selectedClass, onSelectBoard, onDuplicateBoard, onToggleFavorite);
    ...
    return (
        <div className="flex h-full flex-col md:flex-row pb-16 md:pb-0 relative overflow-hidden">
                    ...
                    {filteredBoards.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 text-center opacity-60">
                            ...
                            <p className="text-sm text-gray-400">
                                {selectedClass !== 'All Classes' ? `No boards in ${selectedClass}` : "Try adjusting your filters."}
                            </p>
                        </div>
                    ) : (
                        <div className="pb-20">
            ...
            {menu.visible && (
                <div 
                    ref={menuRef}
                    className="fixed z-50 w-48 bg-[#222] border border-white/10 rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 origin-top-left"
                    style={{ top: menu.y, left: Math.min(menu.x, window.innerWidth - 200) }}
                >
                    <div className="p-1 space-y-0.5">
                        <button onClick={() => handleMenuAction('open')} className="w-full text-left px-3 py-2 text-xs font-bold text-white hover:bg-white/10 rounded-lg flex items-center gap-2">
                            <ExternalLink size={14} /> Open
                        </button>
                        <button onClick={() => handleMenuAction('copyLink')} className="w-full text-left px-3 py-2 text-xs font-bold text-white hover:bg-white/10 rounded-lg flex items-center gap-2">
                            <LinkIcon size={14} /> Copy link
                        </button>
                        <button onClick={() => handleMenuAction('rename')} className="w-full text-left px-3 py-2 text-xs font-bold text-white hover:bg-white/10 rounded-lg flex items-center gap-2">
                            <Edit2 size={14} /> Rename
                        </button>
                        <button onClick={() => handleMenuAction('toggleFav')} className="w-full text-left px-3 py-2 text-xs font-bold text-white hover:bg-white/10 rounded-lg flex items-center gap-2">
                            <Heart size={14} /> Favorite
                        </button>
                        <button onClick={() => handleMenuAction('duplicate')} className="w-full text-left px-3 py-2 text-xs font-bold text-white hover:bg-white/10 rounded-lg flex items-center gap-2">
                    ...
                    <div className="p-1">
                        {(sidebarFilter === 'trashed' || sidebarFilter === 'global_trash') ? (
                            <>
                                <button onClick={() => handleMenuAction('restore')} className="w-full text-left px-3 py-2 text-xs font-bold text-green-400 hover:bg-green-500/10 rounded-lg flex items-center gap-2">
                                    <RotateCcw size={14} /> Restore
                                </button>
                                <button onClick={() => handleMenuAction('hard_delete')} className="w-full text-left px-3 py-2 text-xs font-bold text-red-400 hover:bg-red-500/10 rounded-lg flex items-center gap-2">
                                    <Trash2 size={14} /> Delete Forever
                                </button>
                            </>
                        ) : (
                            <button onClick={() => handleMenuAction('delete')} className="w-full text-left px-3 py-2 text-xs font-bold text-red-400 hover:bg-red-500/10 rounded-lg flex items-center gap-2">
            ...
            <ConfirmModal 
                ...
                confirmText={confirmModal.type === 'soft_delete' ? "Move to Trash" : "Delete Forever"}
                isDangerous={true}
            />
        </div>
    );
