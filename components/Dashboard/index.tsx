...
export const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const NavButton = ({ tab, label, icon: Icon }: { tab: TabView, label: string, icon?: any }) => (
      <button 
        ...
        className={`flex items-center gap-2 px-3 py-2 rounded-lg transition-colors font-bold text-sm ${
            activeTab === tab 
    ...
    <div className={`h-screen flex flex-col ${theme === 'light' ? 'bg-slate-50 text-slate-900' : 'bg-[#111] text-white'} transition-colors duration-300 overflow-hidden`}>
                    ...
                    <div className="absolute top-full left-0 mt-2 w-48 z-50 animate-in fade-in slide-in-from-top-2">
                        <div className="bg-white dark:bg-[#222] rounded-xl shadow-xl border border-gray-200 dark:border-white/10 overflow-hidden max-h-60 overflow-y-auto custom-scrollbar">
                            ...
                            {classList.map((cls) => (
                                <button
                                    key={cls}
                                    onClick={() => { setSelectedClass(cls); setShowClassMenu(false); }}
                                    className={`w-full text-left px-4 py-2 text-xs font-bold hover:bg-gray-100 dark:hover:bg-white/10 transition-colors ${selectedClass === cls ? 'text-pink-500 bg-pink-50 dark:bg-pink-900/10' : 'text-gray-600 dark:text-gray-300'}`}
                                >
                                    {cls}
                                </button>
                            ))}
        ...
        <div className="flex items-center gap-2 md:gap-6 shrink-0">
            ...
            <Tooltip content="Documentation & Guide">
                <button 
                    ...
                    className={`p-2 rounded-full transition-colors hidden md:block ${activeTab === 'documentation' ? 'bg-pink-500/20 text-pink-500' : (theme === 'light' ? 'hover:bg-slate-100 text-slate-500' : 'hover:bg-white/10 text-gray-400 hover:text-white')}`}
                >
                    <BookOpen size={20} />
                </button>
            </Tooltip>
                ...
                <div className="relative">
                    <Tooltip content="Notifications">
                        <button 
                            ...
                            className={`p-2 rounded-full transition-colors hidden md:block relative ${showNotificationMenu ? 'bg-pink-500/20 text-pink-500' : (theme === 'light' ? 'hover:bg-slate-100 text-slate-500' : 'hover:bg-white/10 text-gray-400 hover:text-white')}`}
                        >
                            <Bell size={20} />
                            {unreadCount > 0 && (
                                <div className="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white dark:border-[#1a1a1a]"></div>
                            )}
                        </button>
                    </Tooltip>
                    ...
                    {showNotificationMenu && (
                        <>
                            ...
                            <div className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-[#222] rounded-xl shadow-2xl border border-gray-200 dark:border-white/10 overflow-hidden animate-in fade-in slide-in-from-top-2 z-50">
                                <div className="p-3 border-b border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#1a1a1a] flex justify-between items-center">
                                    <h4 className="font-bold text-sm text-gray-800 dark:text-white">Notifications</h4>
                                    {unreadCount > 0 && (
                                        <button onClick={markAllRead} className="text-[10px] text-blue-500 hover:underline font-bold">Mark all read</button>
                                    )}
                                </div>
                                    ...
                                    {notifications.length === 0 ? (
                                        ...
                                        notifications.map((n) => (
                                            <div 
                                                ...
                                                className={`p-3 border-b border-gray-100 dark:border-white/5 flex gap-3 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors cursor-pointer ${!n.is_read ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}
                                            >
                                                <Avatar src={n.content.avatar} name={n.content.name || 'User'} size="sm" />
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-xs text-gray-800 dark:text-white">
                                                        <span className="font-bold">{n.content.name || 'Unknown User'}</span> joined!
                                                    </p>
                                                    <p className="text-[10px] text-gray-500 truncate">{n.content.email}</p>
                                                    <p className="text-[9px] text-gray-400 mt-1">{new Date(n.created_at).toLocaleTimeString()}</p>
                                                </div>
                                                {!n.is_read && (
            ...
            <Tooltip content={theme === 'dark' ? 'Switch to Light Mode' : 'Switch to Dark Mode'}>
                <button onClick={onToggleTheme} className={`p-2 rounded-full transition-colors hidden md:block ${theme === 'light' ? 'hover:bg-slate-100 text-slate-500' : 'hover:bg-white/10 text-gray-400 hover:text-white'}`}>
            ...
            <div className="relative" ref={profileMenuRef}>
                <Tooltip content={username} position="left">
                    <div 
                        ...
                        className={`w-8 h-8 md:w-9 md:h-9 rounded-full overflow-hidden flex items-center justify-center text-white font-bold text-xs shadow-lg border-2 cursor-pointer shrink-0 hover:scale-105 transition-transform ${theme === 'light' ? 'border-slate-200' : 'border-[#111]'}`}
                    >
                        {userAvatar ? (
                ...
                {showProfileMenu && (
                    <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-[#222] rounded-xl shadow-2xl border border-gray-200 dark:border-white/10 overflow-hidden animate-in fade-in slide-in-from-top-2 z-50">
                        ...
                        <div className="p-1">
                            <button 
                                ...
                                className="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg md:hidden flex items-center gap-2"
                            >
      ...
      <div className="flex-1 overflow-hidden relative pb-16 md:pb-0">
          {activeTab === 'home' && (
             <Home 
                boards={boards}
                onSelectBoard={handleSelectBoardWrapper}
                onDeleteBoard={onDeleteBoard}
                onDuplicateBoard={onDuplicateBoard}
                onToggleFavorite={onToggleFavorite}
                onEmptyTrash={onEmptyTrash}
                onJoinBoard={() => setShowJoinModal(true)}
                onNavigateToMake={() => setActiveTab('make')}
          ...
          {activeTab === 'make' && (
             <div className="h-full w-full overflow-y-auto custom-scrollbar relative">
                <Make onCreateBoard={handleCreateBoardWrapper} theme={theme} />
             </div>
          )}

          {activeTab === 'gallery' && (
             <div className="h-full w-full overflow-y-auto custom-scrollbar relative">
                <Gallery onCreateBoard={handleCreateBoardWrapper} theme={theme} />
             </div>
          )}
          ...
          {activeTab === 'system' && isSuperAdmin && (
             <div className="h-full w-full overflow-y-auto custom-scrollbar relative p-6 md:p-10">
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
              <div className="bg-[#1a1a1a] rounded-xl shadow-2xl w-full max-w-md border border-white/10 p-6">
                  <div className="flex justify-between items-center mb-6">
                      <h3 className="text-xl font-bold text-white">Join a ClassBoard</h3>
                  ...
                  <form onSubmit={handleJoinSubmit} className="space-y-4">
                      <div>
                          <label className="text-xs font-bold text-gray-400 uppercase mb-2 block">Join Code</label>
                          <input 
                            type="text" 
                            value={joinCode}
                            onChange={(e) => setJoinCode(e.target.value.toUpperCase())}
                            maxLength={6}
                            placeholder="e.g. A3F9X2"
                            className="w-full bg-[#111] border border-white/10 rounded-lg p-3 text-white text-lg tracking-widest font-mono text-center focus:border-pink-500 outline-none"
                            autoFocus
                          />
                      ...
                      <button 
                        ...
                        className="w-full bg-pink-600 hover:bg-pink-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3 rounded-lg transition-colors flex items-center justify-center gap-2"
                      >
                          {isJoining && <Activity size={16} className="animate-spin" />}
