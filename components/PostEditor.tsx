...
import { getNoteColorClasses, getColorName } from '../utils/theme';
...
export const PostEditor: React.FC<PostEditorProps> = ({
  ...
  const [color, setColor] = useState<NoteColor>(post?.color || NoteColor.YELLOW);
  const [imageUrl, setImageUrl] = useState(post?.type === 'image' ? post.content : '');
  const [linkUrl, setLinkUrl] = useState(post?.type === 'link' ? post.content : '');
  const [drawingData, setDrawingData] = useState<string | null>(post?.type === 'drawing' ? post.content : null);
  ...
  const draftKey = isCreating && column ? `classboard_draft_${column.id}` : null;
  ...
  const saveDraft = () => {
    if (draftKey) {
      ...
      // Only save if there's some content, to avoid saving empty drafts.
      if (title.trim() || content.trim()) {
        localStorage.setItem(draftKey, JSON.stringify(draft));
      } else {
        localStorage.removeItem(draftKey); // Clean up empty drafts
      }
    }
  };
  ...
  const handlePost = async () => {
    ...
    onClose(); // Note: We call original onClose, not the one that saves draft
  };
  ...
  const handleInsertEmoji = (emoji: string) => {
    if (contentRef.current) {
      ...
      const newContent =
        content.substring(0, selectionStart) + emoji + content.substring(selectionEnd);
      setContent(newContent);
      setShowEmojiPicker(false);
      // Focus and move cursor after the inserted emoji
      setTimeout(() => {
        contentRef.current?.focus();
  ...
  const EditorModeButton: React.FC<{ selfMode: EditorMode; icon: React.ReactNode; label: string }> = ({ selfMode, icon, label }) => (
    <button
      onClick={() => setMode(selfMode)}
      className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold rounded-lg transition-colors ${
        mode === selfMode ? 'bg-white/10 text-white' : 'text-gray-400 hover:bg-white/5 hover:text-white'
      }`}
    >
      {icon} {label}
    </button>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <textarea
                        ...
                        rows={5}
                        className="w-full bg-transparent text-white placeholder-gray-500 focus:outline-none resize-none leading-relaxed custom-scrollbar"
                      />
                       <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="absolute bottom-1 right-1 p-1 rounded-full text-gray-400 hover:bg-white/10 hover:text-white">
                         <Smile size={16} />
                       </button>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-lg py-3 pl-10 pr-4 text-white focus:outline-none focus:border-blue-500"
                      />
                    </div>
                  </div>
                )}
              ...
              <div className="p-4 mt-auto border-t border-white/10 flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-pink-600 hover:bg-pink-500 text-white font-bold rounded-lg transition-all shadow-[0_4px_14px_rgba(219,39,119,0.3)] hover:shadow-[0_6px_20px_rgba(219,39,119,0.4)] disabled:opacity-50"
                  >
