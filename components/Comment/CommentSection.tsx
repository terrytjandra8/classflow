...
import { Comment, CommentAttachment, NoteColor } from '../../types';
...
export const CommentSection: React.FC<CommentSectionProps> = ({ comments, noteId, userId, onAddComment, onUpdateNote, reactionsEnabled, noteColor, isStudent, disablePaste, repliesEnabled, isSectionAnonymous, isReadOnly }) => {
    ...
    const handleLikeComment = (commentId: string) => {
        ...
        onUpdateNote(noteId, { comments: updatedComments });
    };

    const handleDeleteComment = (commentId: string) => {
        if (!onUpdateNote) return;
        const updatedComments = updateCommentInTree(comments, commentId, () => null);
        onUpdateNote(noteId, { comments: updatedComments });
    };

    const handleEditComment = (commentId: string, newText: string) => {
        if (!onUpdateNote) return;
        const updater = (c: Comment) => ({ ...c, text: newText });
        const updatedComments = updateCommentInTree(comments, commentId, updater);
        onUpdateNote(noteId, { comments: updatedComments });
    };
    ...
    const containerClasses = isSolidCard 
        ? `bg-black/5 hover:bg-black/10 border-transparent text-slate-800`
        : `bg-gray-100 dark:bg-white/5 border-transparent text-slate-800 dark:text-white hover:bg-gray-200 dark:hover:bg-white/10`;

    const focusedClasses = isCommentInputFocused 
        ? (isSolidCard ? 'bg-white ring-2 ring-blue-500/20 shadow-sm' : 'bg-white dark:bg-black ring-2 ring-blue-500/50 shadow-sm') 
        : '';
        ...
        <div className="px-4 pb-4 pt-0">
            ...
            {!isReadOnly && (
                <div className="relative w-full mt-2">
                    ...
                    {draftAttachment && (
                        <div className="mb-2 p-2 bg-white dark:bg-[#333] rounded-lg shadow-lg border border-slate-200 dark:border-white/10 flex items-center gap-2 animate-in slide-in-from-bottom-2 fade-in duration-200 w-fit">
                            <div className="w-8 h-8 rounded overflow-hidden bg-slate-100 flex items-center justify-center">
                                {draftAttachment.type === 'image' || draftAttachment.type === 'drawing' ? (
                                    <img src={draftAttachment.content} className="w-full h-full object-cover" />
                                ) : <LinkIcon size={16} className="text-blue-500" />}
                            </div>
                            <span className="text-xs text-slate-500 dark:text-gray-300 truncate max-w-[150px]">Attached</span>
                            <button onClick={() => setDraftAttachment(null)} className="text-slate-400 hover:text-red-500">
                                <X size={14} />
                            </button>
                        </div>
                    )}
                    ...
                    {showAttachmentPicker && (
                        <AttachmentPicker onSelect={handleSelectAttachment} onClose={() => setShowAttachmentPicker(false)} />
                    )}
                    ...
                    <div className={`flex items-end gap-2 rounded-[20px] px-3 py-2 transition-all border ${containerClasses} ${focusedClasses}`}>
                        ...
                        <button 
                            ...
                            className={`h-7 w-7 rounded-full flex items-center justify-center transition-colors shrink-0 ${iconColor} hover:bg-black/5 active:scale-95`}
                        ...
                        <div className="flex-1 py-1">
                            <DynamicTextarea
                                ...
                                onBlur={() => !commentInput && !draftAttachment && setIsCommentInputFocused(false)}
                                onKeyDown={(e) => {
                        ...
                        <div className="flex items-center gap-1 mb-0.5">
                            <button 
                                ...
                                className={`h-7 w-7 flex items-center justify-center rounded-full transition-colors ${iconColor} hover:bg-black/5`}
                            >
                                <Smile size={18} />
                            </button>

                            {(commentInput || draftAttachment) && (
                                <button 
                                    onClick={submitComment}
                                    className="w-7 h-7 flex items-center justify-center text-white bg-blue-500 hover:bg-blue-600 rounded-full transition-all shrink-0 shadow-sm animate-in zoom-in duration-200"
                                >
                        ...
                        {showEmojiPicker && (
                            <div className="absolute bottom-12 right-0 bg-white dark:bg-[#333] shadow-xl rounded-xl border border-slate-200 dark:border-white/10 p-2 z-50 grid grid-cols-5 gap-1 w-48 animate-in zoom-in-95 origin-bottom-right">
                                {EMOJIS.map(emoji => (
                                    <button 
                                        key={emoji} 
                                        type="button"
                                        onClick={() => insertEmoji(emoji)} 
                                        className="w-8 h-8 flex items-center justify-center text-lg hover:bg-slate-100 dark:hover:bg-white/10 rounded transition-colors"
                                    >
                                        {emoji}
                                    </button>
                                ))}
