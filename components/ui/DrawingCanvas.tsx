...
interface DrawingCanvasProps {
    ...
    className?: string;
    strokeColor?: string;
    manualSave?: boolean; // New prop to prevent auto-save
}
...
export const DrawingCanvas: React.FC<DrawingCanvasProps> = ({ 
    ...
    useEffect(() => {
        ...
        if (canvas) {
            ...
            if(ctx) {
                // 1. Fill background with white (fixes transparency issue on save)
    ...
    const saveHistoryStep = () => {
        ...
        if (canvas && ctx) {
            ...
            setHistory(prev => {
                // Discard "future" states if we drew after undoing
            ...
            setHistoryStep(prev => {
                const newHistoryLen = history.slice(0, prev + 1).length + 1; // anticipated length
                return newHistoryLen > 20 ? 19 : newHistoryLen - 1; 
            });
        }
    };

    // Inform parent of changes (for blob upload)
    const notifySave = () => {
        if (manualSave) return; // Do not auto-emit if manualSave is on
    ...
    // Helper to map screen coordinates to canvas coordinates (Supports Touch & Mouse)
    ...
    const startDrawing = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>) => {
        ...
        // Configure stroke based on tool
    ...
    const stopDrawing = () => {
        if (isDrawing) {
            setIsDrawing(false);
            saveHistoryStep(); // Snapshot state after stroke
            notifySave();      // Generate blob (only if !manualSave)
        }
    };
    ...
    const clearCanvas = () => {
        ...
        if (canvas) {
            const ctx = canvas.getContext('2d');
            if (ctx) {
                // Fill with white instead of clearRect to maintain opacity
                ctx.fillStyle = '#ffffff';
                ...
                <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-1">
                        <button
                            ...
                            className="text-xs text-gray-500 hover:text-blue-500 font-bold px-2 py-1.5 rounded-lg transition-colors flex items-center gap-1 disabled:opacity-30 disabled:hover:text-gray-500"
                            title="Undo (Ctrl+Z)"
                        >
                            <Undo size={14} />
                        </button>
                        ...
                        {hasContent && (
                            <>
                                ...
                                <button 
                                    type="button" 
                                    onClick={clearCanvas} 
                                    className="text-xs text-red-500 font-bold hover:bg-red-50 dark:hover:bg-red-900/20 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1"
                                >
                                ...
                                {manualSave && (
                                     <button 
                                        ...
                                        className="text-xs text-white bg-green-600 hover:bg-green-700 font-bold px-4 py-1.5 rounded-lg transition-colors flex items-center gap-1 ml-2"
                                     >
                                         <Check size={14} /> Done
                                     </button>
                                )}
