...
export const ActiveTest: React.FC<ActiveTestProps> = ({ 
    ...
    return (
        ...
        >
            ...
            <div className={`h-16 shrink-0 flex items-center justify-between px-6 border-b z-20 ${isReadingMode ? 'bg-blue-900/20 border-blue-500/30' : (isPracticeMode ? 'bg-teal-900/20 border-teal-500/30' : 'bg-[#161616] border-white/10')}`}>
                ...
                <div className="flex items-center gap-2">
                    {isReadingMode ? (
                        ...
                        <div className="bg-teal-500 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg flex items-center gap-2">
                            <Rocket size={14} /> Practice
                        </div>
                    ) : (
                        <div className="bg-green-500 text-black px-3 py-1 rounded-full text-xs font-bold shadow-lg flex items-center gap-2">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-8 relative">
                <div className="max-w-3xl mx-auto space-y-8 pb-20">
                    {questions.map((q, idx) => {
                        ...
                        return (
                            <QuestionItem 
                                key={q.id} q={q} idx={idx} answer={answers[q.id]}
                                onAnswerChange={(id, val) => onAnswerChange(id, val, false)}
                                isReadingMode={isReadingMode} allowInteractions={allowInteractions}
                                setActiveDrawingQId={setActiveDrawingQId} questionNumber={questionNumber}
                    ...
                    {!isReadingMode && (
                        ...
                        >
                            {isPracticeMode ? <><Check size={20} /> Finish Practice</> : (meetsRequirements ? <><Send size={20} /> {isRevision ? 'Submit Revision' : 'Submit Assessment'}</> : <><AlertTriangle size={20} /> Submit with Warnings</>)}
                        </button>
                    )}
                </div>
            </div>
                    ...
                    <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
                        ...
                        {!meetsRequirements && (
                            <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded-lg text-amber-200 text-xs mb-4 leading-relaxed">
                                <strong>Warning:</strong> You have essay questions that do not meet the minimum word count requirement.
                            </div>
                        )}
                        ...
                        <div className="flex gap-3">
                            ...
                            <button onClick={handleConfirmSubmit} className={`flex-1 py-2.5 rounded-xl font-bold text-sm text-white transition-colors shadow-lg ${meetsRequirements ? 'bg-green-600 hover:bg-green-700' : 'bg-amber-600 hover:bg-amber-700'}`}>
            ...
            {/* FIXED Drawing Modal for Chromebooks */}
            {activeDrawingQId && (
                <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-md flex items-center justify-center p-2 md:p-6">
                     <div className="w-full max-w-6xl h-full max-h-[92vh] flex flex-col bg-[#1a1a1a] border border-white/10 rounded-2xl shadow-2xl overflow-hidden relative">
                         <button 
                            onClick={() => setActiveDrawingQId(null)}
                            className="absolute top-3 right-3 bg-black/50 text-white p-2 rounded-full hover:bg-red-600 transition-colors z-[60]"
                         >
                             <X size={20}/>
                         </button>

                         <div className="flex-1 bg-white relative flex items-center justify-center overflow-hidden p-2">
                             <DrawingCanvas 
                                width={1200}
                                height={800}
