...
export const GradingModal: React.FC<GradingModalProps> = ({ 
    ...
    // Local state for answers (to support manual overrides)
    const [answers, setAnswers] = useState<Record<string, string>>({});
    // Local state for retry/revision flags
    const [retryQuestions, setRetryQuestions] = useState<string[]>([]);
    // Local state for Teacher Overrides
    const [teacherOverrides, setTeacherOverrides] = useState<Record<string, boolean>>({});
    
    // Track formatting of the currently active editor
    ...
    // Ref to hold current state for the debounced function
    ...
    // Manual Debounced Save Function
    const triggerSave = useCallback((overrideAnswers?: Record<string, string>, overrideRetries?: string[], overrideTeacherFlags?: Record<string, boolean>, immediate = false) => {
        ...
        if (immediate) {
            ...
            timeoutRef.current = setTimeout(performSave, 1500); // 1.5s debounce
        }
    }, [onAutoSave]);
    ...
    const execCmd = (cmd: string) => {
        // Prevent default focus loss is handled by onMouseDown in the button itself
        document.execCommand(cmd, false, undefined);
        
        // Force update formats immediately after command using robust check
        setActiveFormats({
            ...
            <div className="bg-[#161616] border border-white/10 rounded-2xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-white/10 flex justify-between items-start bg-[#1a1a1a] rounded-t-2xl gap-4">
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {saveStatus === 'saving' && (
                                <span className="text-[10px] text-yellow-500 flex items-center gap-1 font-bold animate-pulse ml-2">
                                    <Loader2 size={10} className="animate-spin" /> Saving...
                                </span>
                            )}
                            {saveStatus === 'saved' && (
                                <span className="text-[10px] text-green-500 flex items-center gap-1 font-bold ml-2 animate-in fade-in">
                                    <Cloud size={10} /> Saved
                                </span>
                            )}
                            {saveStatus === 'error' && (
                                <span className="text-[10px] text-red-500 flex items-center gap-1 font-bold ml-2">
                                    Error Saving
                                </span>
                            )}
                        </div>
                    ...
                    <div className="flex items-center gap-4 shrink-0 pt-1">
                        <label className="flex items-center gap-2 text-xs font-bold text-gray-400 hover:text-white cursor-pointer select-none transition-colors">
                            <input 
                                ...
                                className="w-4 h-4 rounded border-gray-600 bg-white/5 text-blue-500 focus:ring-0 focus:ring-offset-0"
                            />
                        ...
                        <button 
                            onClick={() => onPrint(includeFeedback, gradesRef.current)} 
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                    {questions.map((q, i) => {
                        ...
                        // Check if upload is allowed (Drawing or Both) AND NO CURRENT ANSWER
                            ...
                            <div key={q.id} className={`bg-[#111] border rounded-xl p-4 transition-colors ${isUnderLimit ? 'border-amber-500/30' : 'border-white/10'}`}>
                                <div className="flex justify-between items-start mb-3">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {isUnderLimit && (
                                            <span className="flex items-center gap-1 text-[10px] font-bold text-amber-500 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20 uppercase tracking-wide">
                                                <FileWarning size={10} /> Word Count
                                            </span>
                                        )}
                                        {isTeacherOverride && (
                                            <span className="flex items-center gap-1 text-[10px] font-bold text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/20 uppercase tracking-wide">
                                                <ShieldCheck size={10} /> Teacher Upload
                                            </span>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-3">
                                        {/* Retry/Revision Toggle */}
                                        <button 
                                            onClick={() => toggleRetry(q.id)}
                                            className={`flex items-center gap-1 px-2 py-1 rounded text-[10px] font-bold uppercase border transition-colors ${isRetrying ? 'bg-orange-500/10 text-orange-500 border-orange-500/30' : 'bg-white/5 text-gray-500 border-transparent hover:text-white'}`}
                                            title={isRetrying ? "Student is redoing this question" : "Unlock for student to redo"}
                                        >
                                        ...
                                        <DebouncedInput 
                                            ...
                                            max={q.points}
                                            value={grade.score}
                                            onChange={(val) => handleUpdate(q.id, { score: parseInt(val) || 0 })}
                                            onFocus={(e: React.FocusEvent<HTMLInputElement>) => e.target.select()}
                                            className="w-16 bg-[#222] border border-white/20 rounded px-2 py-1 text-center font-bold text-white focus:border-blue-500 outline-none"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="bg-[#222] p-3 rounded-lg border border-white/5 mb-4 relative group/answer">
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        {/* Manual Upload Button (Teacher Override) - Only for visual questions AND if empty */}
                                        {canUpload && (
                                            <div className="relative">
                                                ...
                                                <label 
                                                    htmlFor={`upload-${q.id}`}
                                                    className={`flex items-center gap-1.5 cursor-pointer text-[10px] font-bold px-2 py-1 rounded bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/20 transition-all ${isUploading === q.id ? 'opacity-50 pointer-events-none' : ''}`}
                                                >
                                        ...
                                        {isEssay && q.minWords && q.minWords > 0 && (
                                             <span className={`text-[10px] font-bold ${isUnderLimit ? 'text-amber-500' : 'text-green-500'}`}>
                                                 {wordCount} / {q.minWords} valid words
                                             </span>
                                        )}
                                    </div>
                                ...
                                >
                                    ...
                                    {isFocused && (
                                        <div className="flex items-center gap-2 p-2 border-b border-white/5 bg-[#161616] rounded-t-lg animate-in fade-in slide-in-from-top-1">
                                            <button onMouseDown={(e) => { e.preventDefault(); execCmd('bold'); }} className={getBtnClass(activeFormats.bold)} title="Bold">
                                                <Bold size={14}/>
                                            </button>
                                            <button onMouseDown={(e) => { e.preventDefault(); execCmd('italic'); }} className={getBtnClass(activeFormats.italic)} title="Italic">
                                                <Italic size={14}/>
                                            </button>
                                            <div className="w-px h-4 bg-white/10 mx-1"></div>
                                            <button onMouseDown={(e) => { e.preventDefault(); execCmd('subscript'); }} className={getBtnClass(activeFormats.subscript)} title="Subscript">
                                                <Subscript size={14}/>
                                            </button>
                                            <button onMouseDown={(e) => { e.preventDefault(); execCmd('superscript'); }} className={getBtnClass(activeFormats.superscript)} title="Superscript">
                                                <Superscript size={14}/>
                                            </button>
                        ...
                        {isReleased ? (
                            <>
                                <button 
                                    ...
                                    className="px-4 py-2 bg-red-900/20 hover:bg-red-900/40 text-red-400 border border-red-500/30 rounded-lg text-xs font-bold transition-colors flex items-center gap-2"
                                >
                                    <EyeOff size={14}/> Unpublish (Hide)
                                </button>
                                <button 
                                    onClick={() => onSave(true)}
                                    className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs font-bold transition-colors shadow-lg flex items-center gap-2"
                                >
                                    <Check size={14}/> Update Released Results
                                </button>
                            </>
                        ) : (
                            <>
